...
monaco.languages.registerDocumentFormattingEditProvider("glua", new GLuaFormatter());
...
const line = monaco.editor.create(document.getElementById("line-container")!, {
  value: "",
  language: "glua",
  theme: "vs-dark",
  lineNumbers: "off",
...
line.focus();
window.addEventListener("resize", () => {editor.layout(); line.layout()});
...
// @ts-ignore
line._standaloneKeybindingService._getResolver()._lookupMap.get("editor.action.quickCommand")[0].resolvedKeybinding._parts[0].keyCode = 0;
// @ts-ignore
...
widget.showSuggestions = (...args) => {
  ...
  widget.selectLast()
  if(!widget.completionModel || widget.completionModel.hacked){return}
  const oldFn = widget.completionModel._snippetCompareFn
  // @ts-ignore
  widget.completionModel._snippetCompareFn = (...cmpArgs) => {return -oldFn(...cmpArgs)}
...
const widgetStyle = "position: fixed; visibility: inherit; max-width: 1162px; line-height: 19px; bottom: 26px;"
