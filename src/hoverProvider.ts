...
export class GLuaHoverProvider
implements monaco.languages.HoverProvider {
    provideHover(model: monaco.editor.ITextModel, position: monaco.Position, token: monaco.CancellationToken): monaco.languages.ProviderResult<monaco.languages.Hover> {
        ...
        }else if(lineUntil.charAt(word.startColumn - 2)  === ":" && autocompletionData.methodsLookup.has(word.word)) {
            let conent : monaco.IMarkdownString[] = []
            autocompletionData.methodsLookup.get(word.word)?.forEach((method: GluaItem) => {
        ...
        return {
            contents: autocompletionData.valuesLookup.get(currentIdentifier)?.generateDocumentation() || [{value: "No documentation"}],
            range: new monaco.Range( position.lineNumber, firstIdentifierWord.startColumn, position.lineNumber, word.endColumn )
        }
    }
}